...
func init() {
	...
	tr.Dial = func(network, addr string) (net.Conn, error) {
		...
		if err == nil {
			logutil.BgLogger().Debug("rpc: dial new connection", zap.String("addr", addr), zap.Uint64("dials", atomic.AddUint64(&dials, 1)-1))
		}
		return c, err
	}
...
func Send(request *Request) (*Response, error) {
	...
	defer func() {
		...
		// close http response
		rsp.Body.Close()
		logutil.BgLogger().Debug("call rpc", zap.String("method", httpReq.Method), zap.String("URL", httpReq.URL.String()), zap.Duration("time", time.Since(start)))
	}()
